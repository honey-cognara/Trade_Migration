...
async def seed():
    ...
    async with SessionLocal() as db:
        ...
        candidates = [
            CandidateProfile(
                id=electrician_profile_id, user_id=electrician_user_id,
                full_name="Ali Hassan", nationality="Pakistani",
                country_of_residence="Pakistan", trade_category="licensed_electrician",
                is_electrical_worker=True, years_experience=8,
            ...
            CandidateProfile(
                id=plumber_profile_id, user_id=plumber_user_id,
                full_name="Raj Kumar", nationality="Indian",
                country_of_residence="India", trade_category="plumber",
                is_electrical_worker=False, years_experience=5,
                languages=[{"name": "English", "level": "B1"}],
                profile_summary="Experienced plumber with 5 years in commercial fit-outs.",
                published=True,
            ),
        ]
        ...
        employer = EmployerCompany(
            id=employer_company_id, owner_user_id=employer_user_id,
            company_name="VoltCore Electrical Pty Ltd", abn_or_identifier="12 345 678 901",
            contact_name="Sarah Thompson", contact_email="jobs@voltcore.com.au",
            industry="Electrical Contracting", verification_status="approved",
        )
        db.add(employer)
        await db.flush()
        ...
        visa_app = VisaApplication(
            id=uuid.uuid4(), candidate_id=electrician_profile_id,
            employer_company_id=employer_company_id, status="under_review",
        ...
        eoi = ExpressionOfInterest(
            id=uuid.uuid4(), employer_company_id=employer_company_id,
            candidate_id=electrician_profile_id, job_title="Industrial Electrician – Sydney",
            message="We reviewed Ali's profile and believe he is an excellent fit.",
            sponsorship_flag=True, status="unread",
        )
        db.add(eoi)
        ...
        score = ElectricalWorkerScore(
            id=uuid.uuid4(), candidate_id=electrician_profile_id,
            trade_type_score=25, experience_score=20, certification_score=18,
            safety_compliance_score=10, english_score=7, total_score=80, scoring_version="v1.0",
        )
        db.add(score)

        await db.commit()
        print("✅ Demo seed data inserted successfully!")
        print("   Admin:       admin@tradiemigration.com")
        print("   Electrician: ali.hassan@gmail.com")
        print("   Plumber:     raj.kumar@gmail.com")
        print("   Employer:    jobs@voltcore.com.au  (VoltCore Electrical)")
        print("   Case Mgr:    casemanager@tradiemigration.com")
        print("   Agent:       agent@migrationpro.com.au")
